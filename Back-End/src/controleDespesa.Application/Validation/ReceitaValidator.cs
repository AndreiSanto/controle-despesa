...
namespace controleDespesa.Application.Validation
{
    public class ReceitaValidator : AbstractValidator<ReceitaDTO>
    {
        public ReceitaValidator()
        {
            ...
            RuleFor(receita => receita.TipoDespesaReceitaId).GreaterThan(0).WithMessage(MenssagesException.TIPO_RECEITA_INVALIDO);
            RuleFor(receita => receita.Valor).GreaterThan(0).WithMessage(MenssagesException.VALOR_RECEITA_INVALIDO);
